...
<main class="main">
    ...
    <?php get_template_part( 'template-parts/modules/page-header' ); ?>
		...
		<?php if ( $user_favorites ) : ?>
			<?php
			$has_favorites = false; // Initialize a flag to track if any favorites are displayed
			?>

			<?php while ( have_posts() ) : the_post(); ?>
					<?php if ( in_array( get_the_ID(), $user_favorites ) ) : ?>
							<?php $has_favorites = true; // Set the flag to true if a favorite post is found ?>
							<?php break; // Exit loop once a favorite is found ?>
					<?php endif; ?>
			<?php endwhile; ?>
