...
$login_url           = isset( $login_url ) ? $login_url : theme_auth_page_url();
$forgot_password_url = function_exists( 'theme_auth_action_url' ) ? theme_auth_action_url( 'forgot_password' ) : $forgot_password_url;
$catalog_url         = function_exists( 'theme_catalog_page_url' ) ? theme_catalog_page_url() : home_url( '/katalog/' );
$pricing_url         = function_exists( 'theme_pricing_page_url' ) ? theme_pricing_page_url() : home_url( '/cjenik/' );
$account_url         = function_exists( 'theme_account_page_url' ) ? theme_account_page_url() : home_url( '/moj-racun/' );
...
<section class="mepr-auth" aria-labelledby="mepr-auth-title">
	<div class="mepr-auth__card">
		<?php if ( MeprUtils::is_user_logged_in() && ! $admin_view ) : ?>
			...
			<?php else : ?>
				...
				<p class="mepr-auth__eyebrow"><?php esc_html_e( 'Aktivan račun', 'foundationpress' ); ?></p>
				<h1 id="mepr-auth-title"><?php esc_html_e( 'Već si prijavljena', 'foundationpress' ); ?></h1>
				<p class="mepr-auth__intro"><?php esc_html_e( 'Možeš nastaviti na katalog treninga ili urediti podatke računa.', 'foundationpress' ); ?></p>
				<div class="mepr-auth__actions">
					<a class="mepr-auth__button" href="<?php echo esc_url( $catalog_url ); ?>"><?php esc_html_e( 'Idi na katalog', 'foundationpress' ); ?></a>
					<a class="mepr-auth__link" href="<?php echo esc_url( $account_url ); ?>"><?php esc_html_e( 'Moj račun', 'foundationpress' ); ?></a>
				</div>
			<?php endif; ?>
		<?php else : ?>
			<p class="mepr-auth__eyebrow"><?php esc_html_e( 'LOOP račun', 'foundationpress' ); ?></p>
			<h1 id="mepr-auth-title"><?php esc_html_e( 'Prijava', 'foundationpress' ); ?></h1>
			<p class="mepr-auth__intro"><?php esc_html_e( 'Nastavi tamo gdje si stala i otvori svoj katalog treninga.', 'foundationpress' ); ?></p>
			...
			<form name="mepr_loginform" id="mepr_loginform" class="mepr-auth__form mepro-form" action="<?php echo esc_url( $login_url ); ?>" method="post">
				...
				<div class="mp-form-row mepr_password mepr-auth__field">
					<label for="user_pass"><?php esc_html_e( 'Lozinka', 'foundationpress' ); ?></label>
					<div class="mepr-auth__input-group">
						<input type="password" name="pwd" placeholder="<?php esc_attr_e( 'Unesi lozinku', 'foundationpress' ); ?>" id="user_pass" value="" autocomplete="current-password" required>
				...
				<div class="mp-form-row mepr_remember_me mepr-auth__checkbox">
					...
					<label for="rememberme"><?php esc_html_e( 'Zapamti me', 'foundationpress' ); ?></label>
				</div>

				<div class="submit mepr-auth__submit">
					<input type="submit" name="wp-submit" id="wp-submit" class="mepr-auth__button mepr-share-button" value="<?php esc_attr_e( 'Prijavi se', 'foundationpress' ); ?>">
			...
			<div class="mepr-auth__footer">
				<a href="<?php echo esc_url( $forgot_password_url ); ?>"><?php esc_html_e( 'Zaboravljena lozinka?', 'foundationpress' ); ?></a>
				<span><?php esc_html_e( 'Nemaš račun?', 'foundationpress' ); ?> <a href="<?php echo esc_url( $pricing_url ); ?>"><?php esc_html_e( 'Odaberi plan', 'foundationpress' ); ?></a></span>
			</div>
	...
	<aside class="mepr-auth__side" aria-label="<?php esc_attr_e( 'LOOP', 'foundationpress' ); ?>">
		<div class="mepr-auth__quote">
			<?php esc_html_e( 'Žene koje napreduju ne čekaju savršen trenutak. Iskoriste trenutak koji imaju.', 'foundationpress' ); ?>
		</div>
		<ul class="mepr-auth__benefits">
			<li><?php esc_html_e( '200+ treninga po fazama ciklusa', 'foundationpress' ); ?></li>
			<li><?php esc_html_e( 'Live događanja i sezonske kategorije', 'foundationpress' ); ?></li>
			<li><?php esc_html_e( 'Pretplatu možeš otkazati u bilo kojem trenutku', 'foundationpress' ); ?></li>
		</ul>
	</aside>
</section>
