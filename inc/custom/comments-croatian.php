...
function hr_time_ago_for_comment($comment_id, $max_days = 30, $date_format = 'j.n.Y.') {
    ...
    // Use GMT on both sides (note the second param = true on current_time)
    $ts  = get_comment_time('U', true, $c);            // GMT
    $now = current_time('timestamp', true);            // GMT
    $diff = max(0, $now - $ts);

    if ($diff > DAY_IN_SECONDS * $max_days) {
        return date_i18n($date_format, $ts);           // fine: wp_date/date_i18n handles TZ
    }

    if ($diff < 5)                     return 'upravo sada';
    if ($diff < MINUTE_IN_SECONDS)     return 'prije nekoliko sekundi';
